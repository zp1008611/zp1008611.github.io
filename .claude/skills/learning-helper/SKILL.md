...
**处理流程：**
1. 尝试使用 WebFetch 获取每个 URL
2. 如果某个 URL 获取失败（返回错误），记录该 URL 和错误类型
3. **不要跳过失败的 URL**，而是暂停并询问用户：
...
4. 如果用户选择重新输入，等待用户提供新的 URL，替换失败的 URL，然后重新获取
5. 如果用户选择跳过，从列表中移除这些失败的 URL，继续处理其他 URL
