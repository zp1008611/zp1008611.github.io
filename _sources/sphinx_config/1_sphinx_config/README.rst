...
3. 编译为http服务，这样可以通过ip地址查看网页
   ...
   2. 在\ ``zpdoc``\ 中打开cmd重新编译
      ``bash      sphinx-autobuild source build/html`` |image1| ## 3
      更改样式主题

4. 到\ `Sphnix官网 <https://sphinx-themes.org/#themes>`__\ 查看自己想要样式

5. 我使用\ ``Book``\ 主题，安装\ ``sphinx-book-theme``
   ``bash     pip install sphinx-book-theme``

6. 修改\ ``source/conf.py``\ 文件中的\ ``html_theme``\ 字段

   .. code:: python

      html_theme = 'sphinx_book_theme'

7. 在\ ``source/conf.py``\ 文件中的添加以下内容 \```bash source_suffix =
   { ‘.rst’: ‘restructuredtext’, ‘.md’: ‘markdown’ }

   html_title = “My amazing documentation”

   html_theme_options = { “show_navbar_depth”: 1, “max_navbar_depth”: 2,
   “collapse_navbar”: True, “home_page_in_toc”: True,
   “use_download_button”: False, } \``\`

8. 重新编译 ``bash     sphinx-autobuild source build/html``
...
3. 重新编译 ``bash     sphinx-autobuild source build/html`` ## 5
   编辑网页内容
...
4. 修改source文件夹目录结构如下
   ...
   -  ``source/chapter1/index.rst`` \```bash Chapter 1
      =============================
   ...
   -  ``source/chapter2/index.rst`` \```bash Chapter 2
      =============================
   ...
   -  ``source/chapter1/section1/README.md`` ``bash      # S1.1``

   -  ``source/chapter2/section1/README.md`` ``bash      # S2.1``
...
3. 修改\ ``conf.py``\ ，添加
   ``bash     extensions = [...,'sphinx.ext.githubpages',...]``
   这个插件会为生成后的文档添加 .nojekyll 文件， 也会为 GitHub Pages
   自定义域名添加 CNAME 文件。 如果没有 .nojekyll， GitHub Pages 会认为
   \_ 开头的文件夹是 jekyll 内部文件夹， 然后将它过滤掉。

      注意: Sphinx 文档的文件名不能使用大写字母。 因为 Sphinx 在 windows
      上输出时会把的文件名转成小写, 而 GitHub Pages
      的路由是大小写敏感的。
      无法正确链接到使用了大写字母作为文件名的文件。

4. 重新编译 ``bash     sphinx-autobuild source build/html``

5. 将\ ``build/html``\ 的文件推送到仓库的main分支，仓库中\ ``setting``->\ ``page``\ 设置如下：
   |image2| ## 7 注意事项

.. |在这里插入图片描述| image:: https://i-blog.csdnimg.cn/direct/fec23c56b2c34021a68a55a97579f3b4.png
.. |image1| image:: https://i-blog.csdnimg.cn/direct/eb0f2fb92c5b4c068ef9fe7f1e9ad561.png
.. |image2| image:: https://i-blog.csdnimg.cn/direct/96d008a9254544aea08e765dafe0ebed.png
