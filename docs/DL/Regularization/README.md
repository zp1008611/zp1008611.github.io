...
注意，在这两种情况下，我们都对 $y$ 与 $x$ 的关系做了假设. 但我们并不知道真实的关系 $f(x)$ 是什么样的. 训练数据包含100个点. 

我们从训练数据中采样25个点，并训练一个简单模型和一个复杂模型. 重复这个过程‘ $k$ ’次，以训练多个模型（每个模型看到的训练数据样本不同）. 从这些图中我们可以得到一些观察结果. 

在不同数据样本上训练的简单模型之间差异不大. 然而，它们与真实的正弦曲线相差很远（欠拟合）. 另一方面，在不同数据样本上训练的复杂模型之间差异很大（高方差）. 
