...
- DFS：起点入栈->循环：（节点出栈->若节点不在检查集，记录节点，节点的邻居逆序入栈）
- BFS: 起点入队列，起点入检查集-> 循环：（节点从队列左端出来，记录节点，若节点邻居不在检查集，节点的邻居入队列并将节点邻居如检查集）
...
- 动态规划01背包：n个对象，每个对象两个性质，n个对象性质A的和<=C的情况下，最大化n个对象性质B的和；
...
- 动规，拼接return dp最后，换头比较return max(dp)
- 一般返回-1的话，dp初始化全为float('inf')
- 前缀和的维度为n+1
- 10总是和奇偶挂钩
- 堆操作，heap=[],heapq.heappush(heap,elem),heapq.heappop(heap)
- 树/链表，从上往下，向前入递归，从下往上，递归回溯
- 链表中，头节点可能会改变的题目，要设置哨兵节点
